...
awk '{print $1 "_" $2 " " $4/$3}' s1.bn | sort > s1.big

join -1 1 -2 1 -a 2 s1.big s2.big | awk 'BEGIN{prod=1}{if(NF==2) prod=prod*$2; else prod=0}END{print prod}' > prob_s2.txt
join -1 1 -2 1 -a 2 s1.big s3.big | awk 'BEGIN{prod=1}{if(NF==2) prod=prod*$2; else prod=0}END{print prod}' > prob_s3.txt
