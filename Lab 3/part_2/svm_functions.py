...
def train_svm(X, y, parms):
	...
	if parms['kernel'] != 'linear':
		clf = SVC(kernel=parms['kernel'], C=parms['C'], degree=parms['d'],
		          gamma=parms['g'], class_weight={0: 1, 1: 1}, verbose=0, random_state=12345)
	else:
		clf = LinearSVC(C=parms['C'], class_weight={
