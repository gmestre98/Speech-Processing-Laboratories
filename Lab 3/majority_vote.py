...
def main():
	...
	dev_1_pred = directory + '/predictions/egemaps.dev.result.svm.csv'
	test_1_pred = directory + '/predictions/egemaps.test.result.svm.csv'

	dev_2_pred = directory + '/predictions/egemaps.dev.result.nn.csv'
	test_2_pred = directory + '/predictions/egemaps.test.result.nn.csv'

	dev_3_pred = directory + '/predictions/is11.dev.result.svm.csv'
	test_3_pred = directory + '/predictions/is11.test.result.svm.csv'

	dev_4_pred = directory + '/predictions/is11.dev.result.nn.csv'
	test_4_pred = directory + '/predictions/is11.test.result.nn.csv'

	dev_5_pred = directory + '/predictions/avec.dev.result.svm.csv'
	test_5_pred = directory + '/predictions/avec.test.result.svm.csv'

	dev_6_pred = directory + '/predictions/avec.dev.result.nn.csv'
	test_6_pred = directory + '/predictions/avec.test.result.nn.csv'
	...
	preds_devel_5 = pd.read_csv(dev_5_pred)
	preds_devel_5.columns=['file_id', 'predictions_5']
	preds_test_5 = pd.read_csv(test_5_pred)
	preds_test_5.columns=['file_id', 'predictions_5']

	preds_devel_6 = pd.read_csv(dev_6_pred)
	preds_devel_6.columns=['file_id', 'predictions_6']
	preds_test_6 = pd.read_csv(test_6_pred)
	preds_test_6.columns=['file_id', 'predictions_6']

	# Merge all predictions and labels (if available) as columns of the same dataframe:
	devel = pd.merge(pd.merge(pd.merge(pd.merge(pd.merge(pd.merge(labels_devel, preds_devel_1, on='file_id'), preds_devel_2, on='file_id'), preds_devel_3, on='file_id'), preds_devel_4, on='file_id'), preds_devel_5, on='file_id'), preds_devel_6, on='file_id')
	test = pd.merge(pd.merge(pd.merge(pd.merge(pd.merge(pd.merge(labels_test, preds_test_1, on='file_id'), preds_test_2, on='file_id'), preds_test_3, on='file_id'), preds_test_4, on='file_id'), preds_test_5, on='file_id'), preds_test_6, on='file_id')
	...
	print(devel['predictions_5'])
	print(devel['predictions_6'])
	print(devel.shape[0])
	print(test['predictions_1'])
	print(test['predictions_2'])
	print(test['predictions_3'])
	print(test['predictions_4'])
	print(test['predictions_5'])
	print(test['predictions_6'])
	print(test.shape[0])
	...
	for i in range (0, devel.shape[0]-1):
		...
		if devel['predictions_4'][i] == 1:
			count_1 = count_1 + 1
		if devel['predictions_5'][i] == 1:
			count_1 = count_1 + 1
		if devel['predictions_6'][i] == 1:
			count_1 = count_1 + 1
		if count_1 > 3:
			res[i] = 1
		else:
	...
	print("Results for prediction 5")
	f1 = precision_recall_fscore_support(devel.Label.values, devel.predictions_5.values, labels=[0,1], average='macro')
	print(f1)

	print("Results for prediction 6")
	f1 = precision_recall_fscore_support(devel.Label.values, devel.predictions_6.values, labels=[0,1], average='macro')
	print(f1)
	...
	for i in range (0, test.shape[0]-1):
		...
		if test['predictions_4'][i] == 1:
			count_1 = count_1 + 1
		if test['predictions_5'][i] == 1:
			count_1 = count_1 + 1
		if test['predictions_6'][i] == 1:
			count_1 = count_1 + 1
		if count_1 > 3:
			res2[i] = 1
		else:
